...
simple_neighbor_cases = {
    "TC1_simple_neighbors": {
        ...
        "expected_neighbors_for_bin_4": [
            # Simple case: Index 4 = (1,1), should return its
            # 8-connected surrounding bins
            [(0, 1)],
            [(1, 0)],
            [(1, 2)],
            [(2, 1)],
    ...
    "TC2_neighbors_higher": {
        ...
        "expected_neighbors_for_bin_4": [
            # Target Bin (1,1) is surrounded by same-valued bins,
            # each in separate superbin
            [(0, 1)],
            [(1, 0)],
            [(1, 2)],
            [(2, 1)],
...
stress_cases = {
    "TC6_large_histogram_center": {
        ...
        "bin_contents": np.pad(
            np.ones((3, 3)), pad_width=3, mode="constant", constant_values=5
        ),
        "target_bin": (4, 4),
        "expected_neighbors": [
            [(3, 4)],
    ...
    "TC7_high_values_3x3": {
        ...
        "bin_contents": np.array([[100, 100, 100], [100, 1, 100], [100, 100, 100]]),
        "target_bin": (1, 1),
        "expected_neighbors": [
            [(0, 1)],
            [(1, 0)],
    ...
    "TC8_isolated_high_center": {
        ...
        "bin_contents": np.array([[1, 1, 1], [1, 100, 1], [1, 1, 1]]),
        "target_bin": (1, 1),
        "expected_neighbors": [
            [(0, 1)],
            [(1, 0)],
...
def test_get_neighbor_bins(name, case):
    ...
    assert (
        neighbors == case["expected_neighbors_for_bin_4"]
    ), f"Failed {name}: Expected {case['expected_neighbors_for_bin_4']}, \
          got {neighbors}"
...
def test_edge_and_corner_neighbors(name, case):
    ...
    assert (
        neighbors == case["expected_neighbors"]
