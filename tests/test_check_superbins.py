...
def create_merger(hist_shape, superbin_indices):
    ...
    merger = AsymBinMerger(
        hist=dummy_hist, max_stat_uncert=0.1, output_dir=".", debug=True
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            ...
            [(1, 0), (1, 1), (2, 2)],
        ],  # multiple superbins with varied lengths
        [
            [(0, 0)],
            [(1, 1), (1, 2)],
            [(2, 0), (2, 1)],
        ],  # multiple superbins with mixed lengths
    ],
)
...
def test_check_superbins_invalid_structure(superbins):
    if not all(isinstance(sb, list) for sb in superbins):
        with pytest.raises(
            TypeError, match="superbin_indices must be a list of lists."
        ):
